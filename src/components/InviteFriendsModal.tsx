  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-xs overflow-y-auto" id="invite-friends-modal-backdrop">
      <motion.div
        ...
        className="bg-white rounded-3xl max-w-md w-full overflow-hidden shadow-2xl border border-[#D8E2DC] my-6"
        id="invite-friends-modal-container"
      >
        {/* Header */}
        <div className="bg-[#1B4332] text-white p-6 relative text-center space-y-2">
          <button
            onClick={onClose}
          ...
          <div className="w-12 h-12 rounded-2xl bg-[#52B788] text-white flex items-center justify-center mx-auto shadow-md">
          ...
          <p className="text-xs text-[#D8F3DC]/90 max-w-xs mx-auto">
        ...
        <div className="p-6 space-y-5 text-xs">
          ...
          {!referralConfig.isEnabledByAdmin ? (
            ...
            <div className="p-3.5 bg-[#F0F7F4] rounded-2xl border border-[#74C69D] text-[#1B4332] text-[11px] leading-relaxed flex items-start gap-2">
          ...
          <div className="space-y-1.5">
            <label className="block font-bold text-[#1B4332]">Your Unique Referral Code</label>
            <div className="flex items-center justify-between p-3.5 bg-[#F8FAF9] rounded-2xl border border-[#D8E2DC]">
              <span className="text-base font-black text-[#1B4332] tracking-wider font-mono">
                {userReferral.referralCode}
              </span>
              <button
                onClick={handleCopyLink}
                className="px-3 py-1.5 rounded-xl bg-[#2D6A4F] hover:bg-[#1B4332] text-white font-bold text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <button
              onClick={handleCopyLink}
              className="py-3 px-4 rounded-2xl bg-[#F0F7F4] hover:bg-[#E9F5EE] text-[#1B4332] font-bold text-xs border border-[#D8E2DC] flex items-center justify-center gap-2 transition-colors cursor-pointer"
            >
          ...
          <div className="grid grid-cols-3 gap-2 text-center pt-2">
            <div className="p-3 rounded-2xl bg-[#F8FAF9] border border-[#D8E2DC]">
              <p className="text-[10px] text-gray-500 font-bold uppercase">Invited</p>
              <p className="text-base font-black text-[#1B4332]">{userReferral.totalInvited}</p>
            </div>
            <div className="p-3 rounded-2xl bg-[#F8FAF9] border border-[#D8E2DC]">
              <p className="text-[10px] text-gray-500 font-bold uppercase">Subscribed</p>
              <p className="text-base font-black text-[#2D6A4F]">{userReferral.activeSubscribers}</p>
            </div>
            <div className="p-3 rounded-2xl bg-[#F8FAF9] border border-[#D8E2DC]">
