...
export const DriverApp: React.FC<DriverAppProps> = ({
  ...
  return (
    <div className="space-y-6 w-full max-w-full box-border" id="driver-app-root" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-[#1B4332] text-white rounded-3xl p-4 sm:p-6 lg:p-7 shadow-sm border border-[#2D6A4F]/40 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-5 box-border">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          <div className="relative shrink-0">
            <img
              ...
              className="h-14 w-14 sm:h-16 sm:w-16 rounded-2xl object-cover border-2 border-[#74C69D] shadow-md"
            />
            <span className={`absolute -bottom-1 -right-1 h-3.5 w-3.5 sm:h-4 sm:w-4 rounded-full border-2 border-[#1B4332] ${
          ...
          <div className="min-w-0 text-start">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-[11px] sm:text-xs font-bold px-2.5 py-0.5 rounded-full bg-white/15 text-[#D8F3DC] border border-white/20 whitespace-nowrap">
                ★ {currentDriver.rating} ({formatNumber(currentDriver.totalDeliveries, language)} {translate('trips', language)})
              </span>
            </div>

            <p className="text-[11px] sm:text-xs text-[#D8F3DC]/90 mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5">
              <span>{currentDriver.vehiclePlate}</span>
              <span>•</span>
              <span>{getVehicleLabel(currentDriver.vehicleType)}</span>
              <span>•</span>
              <span className="text-[#74C69D] font-bold">{translate('onTime', language)}: {currentDriver.onTimeRate}%</span>
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 box-border">
        <div className="bg-white rounded-2xl p-4 border border-[#D8E2DC] shadow-xs text-start">
          <p className="text-[11px] font-bold text-neutral-400 uppercase tracking-wider">{translate('todaysEarnings', language)}</p>
        ...
        <div className="bg-white rounded-2xl p-4 border border-[#D8E2DC] shadow-xs text-start">
          <p className="text-[11px] font-bold text-neutral-400 uppercase tracking-wider">{translate('completedDeliveries', language)}</p>
          <p className="text-lg font-black text-[#1B4332] mt-0.5">{formatNumber(6, language)} {translate('ordersUnit', language)}</p>
        </div>

        <div className="bg-white rounded-2xl p-4 border border-[#D8E2DC] shadow-xs text-start">
          <p className="text-[11px] font-bold text-neutral-400 uppercase tracking-wider">{translate('coldChainSensor', language)}</p>
        ...
        <div className="bg-white rounded-2xl p-4 border border-[#D8E2DC] shadow-xs text-start">
          <p className="text-[11px] font-bold text-neutral-400 uppercase tracking-wider">{translate('safetyRating', language)}</p>
          <p className="text-lg font-black text-[#2D6A4F] mt-0.5">{translate('tamperFree100', language)}</p>
        </div>
      </div>

      {/* Active Trip Dispatch Card */}
      {activeTrip ? (
        <div className="bg-white rounded-3xl p-4 sm:p-6 lg:p-7 border border-[#D8E2DC] shadow-xs space-y-6 box-border">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-100 pb-4 text-start">
            <div>
              ...
              <h2 className="text-lg sm:text-xl font-black text-[#1B4332] mt-1.5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 box-border text-start">
            ...
            <div className="p-4 rounded-2xl bg-[#F4F7F5] border border-[#D8E2DC] space-y-3 box-border">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1">
                  <Package className="w-3.5 h-3.5 text-[#2D6A4F] shrink-0" />
                  <span>{translate('step1OriginPharmacy', language)}</span>
                </span>
                <span className="text-xs font-bold text-[#2D6A4F]">{translate('pickupReady', language)}</span>
              </div>
              ...
              <div className="p-3 rounded-xl bg-white border border-[#D8E2DC] text-xs text-neutral-700 flex items-center justify-between">
                ...
                <span className="font-mono font-bold text-[#1B4332]">#DAWA-SEC-{selectedCountry.code}-8492</span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-[#F4F7F5] border border-[#D8E2DC] space-y-3 box-border">
              ...
              <div className="p-3 rounded-xl bg-white border border-[#D8E2DC] text-xs text-neutral-700 flex items-center justify-between">
                ...
                <span className="font-bold text-[#1B4332]">{activeTrip.estimatedDeliveryMinutes || 20} {translate('minutes', language)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-6 rounded-3xl bg-[#F4F7F5] border border-[#D8E2DC] space-y-4 box-border text-start">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#D8E2DC] pb-3">
              ...
              {/* Handover Mode Tabs */}
              <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-[#D8E2DC] shrink-0">
                <button
                  type="button"
            ...
            {deliveredSuccess ? (
              <div className="py-6 text-center text-xs">
                ...
                <h4 className="text-base font-black text-[#1B4332]">{translate('deliveryConfirmedFinalized', language)}</h4>
              ...
              <form onSubmit={handleDeliveryConfirm} className="space-y-4 text-xs">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                      <div className="relative w-full sm:max-w-xs">
                        ...
                        <input
                          ...
                          className={`w-full py-2.5 rounded-2xl bg-white font-mono text-base font-black tracking-widest text-[#1B4332] border border-[#D8E2DC] focus:ring-2 focus:ring-[#2D6A4F] focus:outline-none ${isRtl ? 'pr-10 pl-4' : 'pl-10 pr-4'}`}
                        />
                      </div>
                ...
                {handoverMode === 'qr' && (
                  <div className="p-4 rounded-2xl bg-white border border-[#D8E2DC] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-[#1B4332]">{translate('scanRecipientAppCode', language)}</h4>
                ...
                {handoverMode === 'signature' && (
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 rounded-xl bg-white border border-[#D8E2DC] text-xs focus:ring-2 focus:ring-[#2D6A4F] focus:outline-none"
                    />
                  </div>
                )}
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  <button
                    ...
                    className="w-full sm:w-auto px-6 py-3 rounded-2xl bg-[#1B4332] hover:bg-[#2D6A4F] text-white text-xs font-black transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm disabled:opacity-50"
                  >
                    {isVerifying ? (
        ...
        <div className="bg-white rounded-3xl p-8 sm:p-12 text-center border border-[#D8E2DC] box-border">
          <Truck className="w-12 h-12 text-neutral-300 mx-auto mb-3" />
          <h3 className="text-base font-bold text-[#1B4332]">{translate('noAssignedTripsTitle', language)}</h3>
          <p className="text-xs text-neutral-500 mt-1">{translate('noAssignedTripsSubtitle', language)}</p>
        </div>
      )}
        ...
        {isRegisterDriverOpen && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto" dir={isRtl ? 'rtl' : 'ltr'}>
            <motion.div
              ...
              className="bg-white rounded-3xl max-w-lg w-full p-5 sm:p-7 shadow-2xl border border-[#D8E2DC] my-8 box-border"
            >
              <div className="flex items-center justify-between border-b border-neutral-100 pb-3 mb-4">
                <div className="flex items-center gap-2">
                  <Bike className="w-5 h-5 text-[#2D6A4F]" />
                  <h3 className="text-base font-black text-[#1B4332]">{translate('registerCourierModalTitle', language)}</h3>
                </div>
              ...
              {regSuccess ? (
                <div className="py-6 text-center text-xs">
                  ...
                  <h4 className="text-sm font-bold text-[#1B4332]">{translate('driverProfileRegistered', language)}</h4>
                  <p className="text-neutral-500 mt-1">{translate('driverActiveInFleet', language)}</p>
                </div>
              ) : (
                <form onSubmit={handleRegisterDriver} className="space-y-3.5 text-xs text-start">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 rounded-2xl bg-[#F4F7F5] border border-[#D8E2DC] focus:ring-2 focus:ring-[#2D6A4F] focus:outline-none"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3.5 py-2.5 rounded-2xl bg-[#F4F7F5] border border-[#D8E2DC] font-mono focus:ring-2 focus:ring-[#2D6A4F] focus:outline-none"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-neutral-100">
                    ...
                    <button
                      type="submit"
                      className="px-5 py-2.5 rounded-2xl bg-[#1B4332] hover:bg-[#2D6A4F] text-white font-bold shadow-sm cursor-pointer"
                    >
