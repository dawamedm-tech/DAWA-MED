        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4 text-xs max-h-[75vh] overflow-y-auto">
          ...
          {availableMedicines.length > 0 && !existingReminder && (
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 max-h-20 overflow-y-auto p-1.5 bg-[#F1FAF4] rounded-2xl border border-[#E8F5EE]">
                {availableMedicines.slice(0, 8).map((m) => (
                  <button
                    key={m.id}
                    type="button"
                    onClick={() => handleQuickSelectMedicine(m)}
                    className="px-2.5 py-1 rounded-xl bg-white hover:bg-[#E8F5EE] border border-[#E8F5EE] text-[11px] font-bold text-[#111827] transition-colors"
                  >
                    + {m.name}
                  </button>
                ))}
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-[#E8F5EE] bg-white text-xs font-bold text-[#111827] focus:ring-2 focus:ring-[#0E7A4B] focus:outline-none"
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-[#E8F5EE] bg-white text-xs text-[#111827] focus:ring-2 focus:ring-[#0E7A4B] focus:outline-none"
          ...
          {familyMember === 'Other' && (
            <div>
              <label className="block font-bold text-[#111827] mb-1">Custom Name / Relationship</label>
              <input
                type="text"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-[#E8F5EE] bg-white text-xs font-mono text-[#111827]"
              />
            </div>
            ...
            <div>
              ...
              <div className="space-y-1.5">
                ...
                />
                <label className="flex items-center gap-1.5 cursor-pointer text-[11px] text-[#111827] font-bold">
                  <input
                    type="checkbox"
                    checked={isOngoing}
                    onChange={(e) => setIsOngoing(e.target.checked)}
                    className="accent-[#0E7A4B] rounded-sm"
                  />
          ...
          <div className="space-y-2 p-3.5 bg-[#F1FAF4] rounded-2xl border border-[#E8F5EE]">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {reminderTimes.map((time) => (
                <span
                  key={time}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-[#E8F5EE] shadow-xs text-xs font-black text-[#111827]"
                >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="px-3 py-1.5 rounded-xl border border-[#E8F5EE] bg-white text-xs font-mono w-28"
                placeholder="02:00 PM"
              />
              <button
                type="button"
                onClick={handleAddReminderTime}
                className="px-3 py-1.5 bg-[#0E7A4B] hover:bg-[#0B6B43] text-white font-bold rounded-xl flex items-center gap-1 cursor-pointer"
              >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2 rounded-xl border border-[#E8F5EE] bg-white text-xs text-[#111827]"
              placeholder="e.g. Keep in cool dry place away from children. Take after breakfast."
            />
          </div>

          {/* Privacy Shield Toggle */}
          <div className="p-3 bg-[#F1FAF4] rounded-2xl border border-[#E8F5EE] flex items-center justify-between">
            <div className="space-y-0.5 pr-2">
              <div className="flex items-center gap-1.5 font-bold text-[#111827]">
                <EyeOff className="w-3.5 h-3.5 text-[#0E7A4B]" />
          ...
          {/* Footer Submit */}
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-[#E8F5EE]">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2.5 bg-[#F1FAF4] hover:bg-[#E8F5EE] text-[#111827] rounded-2xl font-bold border border-[#E8F5EE] cursor-pointer"
            >
              {t.cancel}
            </button>
            <button
              type="submit"
              className="px-6 py-2.5 bg-[#0E7A4B] hover:bg-[#0B6B43] text-white rounded-2xl font-bold cursor-pointer transition-colors shadow-xs"
