    ...
    try {
      ...
      if (!response.ok || !data?.success) {
        if (data?.code === 'FACEBOOK_CREDENTIALS_REQUIRED' || response.status === 501) {
          setErrorMsg(data?.error || 'NOT VERIFIED — META/FACEBOOK CREDENTIALS REQUIRED: يلزم تفعيل مزود Facebook وإدخال معرف التطبيق (App ID) والسر (App Secret) في لوحة Firebase Console.');
        } else {
          setErrorMsg(
            data?.error || 
    ...
    <AuthLayout onClose={handleClose} isRtl={isRtl}>
      ...
      {authMode === 'login' && (
        <div className="w-full flex flex-col items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 w-full" id="login-social-row">
            {/* Google Sign In Button */}
            <button
              type="button"
              onClick={handleGoogleSignIn}
              disabled={isLoading}
              className="w-full h-[41px] rounded-[11px] bg-white border border-[#DFE8F6] hover:bg-[#F8FAFC] flex items-center justify-center gap-2 shadow-2xs transition-all active:scale-98 cursor-pointer text-[12.5px] font-medium text-[#374151] disabled:opacity-50"
              title="Google"
            ...
            >
              ...
              <span>{isRtl ? 'المتابعة بـ Google' : 'Google'}</span>
            </button>
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full h-[41px] rounded-[11px] bg-[#1877F2] hover:bg-[#166fe5] text-white border border-[#1877F2] flex items-center justify-center gap-2 shadow-2xs transition-all active:scale-98 cursor-pointer text-[12.5px] font-medium disabled:opacity-50"
              title="Facebook"
            ...
            >
              ...
              <span>{isRtl ? 'المتابعة بـ Facebook' : 'Facebook'}</span>
            </button>
          </div>
      ...
      {authMode === 'register' && (
        <div className="w-full flex flex-col items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 w-full" id="register-social-row">
            <button
              type="button"
              onClick={handleGoogleSignIn}
              disabled={isLoading}
              ...
              id="register-social-google-button"
            >
              <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M23.745 12.27c0-.7-.06-1.4-.19-2.07H12v4.51h6.6c-.29 1.52-1.14 2.82-2.4 3.68v3.05h3.88c2.27-2.09 3.665-5.17 3.665-9.17z" />
                <path fill="#34A853" d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.88-3.05c-1.08.72-2.45 1.16-4.05 1.16-3.12 0-5.77-2.1-6.72-4.93H1.26v3.15C3.25 21.36 7.34 24 12 24z" />
            ...
            <button
              ...
              id="register-social-facebook-button"
            >
              <svg className="w-4 h-4 shrink-0 fill-current" viewBox="0 0 24 24">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
              </svg>
              <span>{isRtl ? 'Facebook' : 'Facebook'}</span>
            </button>
          </div>
